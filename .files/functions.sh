...
function dbs {
  ...
  echo php artisan -vvv db:seed $_class
  php artisan -vvv db:seed $_class
}
...
git_prompt_info() {
  ref=$( ( cat .git/HEAD | sed -E 's/.*\/heads\/(.+)/\1/' ) 2>/dev/null ) || return
  if [[ ! -z $ref ]]; then
...
function netsize {
  VAL=$(curl -sIL $1 | awk '/[Cc]ontent-[Ll]ength/ {print $2}' | tr -d '\r')
